...
/// <summary>Expected pathogens per type over cycles 1..n, used to size the daily missions.</summary>
public sealed record SpawnTotals(double Basic, double Fast, double Tank, double Ranged, double Total, double Boss);
...
public static class SpawnBalance
{
    ...
    public static bool IsBossCycle(int cycle) => cycle > 0 && cycle % CycleBalance.BossEvery == 0;

    /// <summary>Expected spawns per type over cycles 1..cycle. Normalised by the weight sum, not by 100.</summary>
    public static SpawnTotals Totals(double cycle)
    {
        double basic = 0, fast = 0, tank = 0, ranged = 0, total = 0;
        ...
        return new SpawnTotals(basic, fast, tank, ranged, total, Math.Floor((double)last / CycleBalance.BossEvery));
    }
}
